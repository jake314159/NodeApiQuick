...
ApiQuick.prototype = {
    ...
    init: function(port, extra) {
        ...
        if(extra.headers !== undefined) {
            this.headers = extra.headers;
        }
        if(extra.cacheControl !== undefined) {
            this.cacheControl = extra.cacheControl;
        }
            ...
            for(var k in d) {
                ...
                if(typeof d[k] == 'function') {
                    ...
                    if(extra && extra.headers) {
                        d[k].headers = extra.headers;
                    }
                    if(extra && typeof extra.cacheControl !== "undefined") {
                        d[k].cacheControl = extra.cacheControl;
                    }
        ...
        while(!returnDict.ok && depth <= this.maxDepth) {
            if(this.routes[endpoint]) {
                ...
                if(endIndex !== 0) {
                    ...
                    endpoint = endpoint.substring(0, endIndex);
                } else {
                    break;
                }
            }
     ...
     */
    _getHeaders: function(data, extra) {
        var header = {
        ...
        // Handle cacheing
        var cacheObj = this.cacheControl
        if(extra && extra.handler && typeof extra.handler.cacheControl !== "undefined") {
            cacheObj = extra.handler.cacheControl;
        }
        var cacheControl = this._generateCacheHeader(cacheObj);
        if(cacheControl) {
        ...
        if(extra && extra.handler && extra.handler.headers) {
            var endpointHeaders = extra.handler.headers;
            if(typeof endpointHeaders === "function") {
            ...
            for(var key in endpointHeaders) {
                if(endpointHeaders === null) {
                    ...
                    header[key] = endpointHeaders[key];
                }
            }
        }
        return header;
    ...
    _generateCacheHeader: function(cacheObj) {
        var cacheHeaderParts = [];

        // Convert to an object if it is not
        if(cacheObj === false || cacheObj === 0 || this.debug) {
            // Cacheing disabled
            cacheObj = {noCache: true, noStore: true, mustRevalidate: true};
        } else if(typeof cacheObj === "number") {
            // Number which is treated as the maxAge
            cacheObj = {public: true, maxAge: cacheObj};
        } else if(typeof cacheObj === "string") {
            // Already a header string
            return cacheObj;
        } else if(typeof cacheObj !== "object" || cacheObj === null) {
            // Default
            cacheObj = {public: true, maxAge: 30};
        }
        
        // Where can it be cached
        if(cacheObj.public) {
            cacheHeaderParts.push('public');
        } else if(cacheObj.private) {
            cacheHeaderParts.push('private');
        } else if(cacheObj.noCache) {
            cacheHeaderParts.push('no-cache');
        } else if(cacheObj.onlyIfCached) {
            cacheHeaderParts.push('only-if-cached');
        }

        // Add all the other flags/paramiters
        if(cacheObj.noStore) {
            cacheHeaderParts.push('no-store');
        }
        if(cacheObj.mustRevalidate) {
            cacheHeaderParts.push('must-revalidate');
        }
        if(cacheObj.immutable) {
            cacheHeaderParts.push('immutable');
        }
        if(cacheObj.noTransform) {
            cacheHeaderParts.push('no-transform');
        }
        if(cacheObj.maxAge) {
            cacheHeaderParts.push('max-age='+cacheObj.maxAge);
        }
        if(cacheObj.sharedMaxAge) {
            cacheHeaderParts.push('s-max-age='+cacheObj.sharedMaxAge);
        }
        if(cacheObj.maxStale) {
            cacheHeaderParts.push('max-stale='+cacheObj.maxStale);
        }
        if(cacheObj.minFresh) {
            cacheHeaderParts.push('min-fresh='+cacheObj.minFresh);
        }
        if(cacheObj.staleWhileRevalidate) {
            cacheHeaderParts.push('stale-while-revalidate='+cacheObj.staleWhileRevalidate);
        }
        if(cacheObj.staleIfError) {
            cacheHeaderParts.push('stale-if-error='+cacheObj.staleIfError);
        }
        return cacheHeaderParts.join(', ');
    },
    ...
    _writeResponse: function(res, data, extra) {
        if(!data) data = '' + data;
        if(!extra) extra = {};
        var header = this._getHeaders(data, extra);
        ...
        return function(req, res) {
            try {
                self.middleware.run(req, res, function() {
                    ...
                    if(!responseData.ok) {
                        return self._writeResponse(res, responseData, {handler: handler});
                    }
                    ...
                    self._checkAuthDetails(handler.auth, authDetails, function(auth) {
                        if(!auth) {
                            return self._writeResponse(res, {code: 401, ok: false, error: "Auth failed"}, {handler: handler});
                        }
                        ...
                        setTimeout(function() {
                            try{
                                if(handler.length >= 2) {
                                    ...
                                    self._writeResponse(res, result, {handler: handler});
                                }
                            } catch(e) {
