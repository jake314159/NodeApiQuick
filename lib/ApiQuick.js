...
ApiQuick.prototype = {
    ...
    checkAuthDetails: function(handler_auth, authDetails, cb) {
        if(handler_auth) {
     ...
     * @returns: 
     */
    writeResponse: function(res, data, extra) {
        if(!data) data = '' + data;
        if(!extra) extra = {};
        ...
        return function(req, res) {
            try {
                self.middleware.run(req, res, function() {
                    ...
                    var responseData = self.checkParamsSupplied(req);

                    if(!responseData.ok) {
                        return self.writeResponse(res, responseData);
                    }
                    ...
                    self.checkAuthDetails(handler.auth, authDetails, function(auth) {
                        if(!auth) {
                            self.logger.emit('debug', 'Authentication fail', {credentials: authDetails});
                            return self.writeResponse(res, {code: 401, ok: false, error: "Auth failed"});
                        }
                        ...
                        setTimeout(function() {
                            try{
                                if(handler.length >= 2) {
                                    ...
                                    handler({method: method, args: args, body: req.body}, function(err, result, extra) {
                                        self.writeResponse(res, result, extra);
                                    });
                                } else {
                                    // Handler doesn't take a callback so run synchronously
                                    var result = handler({method: method, args: args, body: req.body});
                ...
                self.logger.emit('error', 'Uncaught exception', {e: e, stack: e.stack});
                self.writeResponse(res, {
                    ok: false,
                    code: 500,
