...
ApiQuick.prototype = {
	...
	init: function(port, extra) {
		...
		this.middleware = [];  // TODO: Run the things in here
		...
		// Create handler
		var processRequest = this.process('GET', function(req){return req.query})
	...
	checkParamsSupplied: function(req) {
		...
		if(!this.packages[pack]) {
			...
			returnDict.ok = false;
			returnDict.code = 405;
			returnDict.error = "No function "+func+" in package"
		} else {
	...
	process: function(method, getData) {
		...
		return function(req, res) {
			...
			req.u = u;
			if(!req.body) req.body = req.u.query; // If no POST data use the query string (for GETs)
			
			// TODO: TEST above f POST put post data into req.u.query
			...
			try {
				...
				if(!responseData.ok) {
					...
					setTimeout(function() {
						try{
							var functionResponseData = handler(method, args, req.body)
